...
- **Correção — `validarPeriodoAberto is not defined`.** A entrega anterior incluiu no pacote de alterados as páginas que **chamam** a trava de período, mas não o `assets/dados.js`, que a **define** — com a atualização parcial, qualquer lançamento quebrava. Além do arquivo correto, as páginas passaram a chamar a trava por um invólucro tolerante: se a função central não estiver carregada, o lançamento segue e a **trava do banco continua valendo**, de modo que uma atualização parcial nunca mais impeça o trabalho.

- **Trava de período fechado no banco (`trava_periodo_fechado.sql`) e em todas as telas.** A trava existia apenas na dispensação e na transferência, e só como validação de tela — ajustes, notas fiscais, doações, devoluções e entradas de custódia passavam pelo fechamento, e qualquer gravação fora da interface também. Agora há um **gatilho no banco** aplicado a `dispensacoes`, `devolucoes`, `ajustes_estoque`, `notas_fiscais`, `doacoes`, `medicacao_propria`, `inventario_inicial`, `custodia_destinos` e `transferencias_custodia`, que recusa qualquer lançamento com data igual ou anterior ao fechamento — inclusive pelo SQL Editor. A validação de tela foi estendida aos mesmos pontos, para o usuário ver uma mensagem clara em vez do erro cru do banco. Sem fechamento configurado, nada é bloqueado.

- **Fechamento do mês na tela do BMPO.** O controle existia apenas em Configurações, longe de onde a escrituração é concluída. Passou a existir também **na própria tela do BMPO**, abaixo do balanço: um botão **🔒 Fechar &lt;mês&gt;** que fecha até o último dia do mês em exibição, com o aviso de conferir o balanço e a Folha de Registro Semanal antes. Quando o mês já está fechado, o painel mostra a data e oferece **Reabrir período**. Os dois controles operam o mesmo campo.
