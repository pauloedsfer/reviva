-- ============================================================================
-- Hospital Reviva — pops_conteudo_v4.sql   (revisado)
-- Acrescenta o POP-FAR-001 — Sistema de Gestão da Qualidade da Farmácia.
-- POP mestre: rege elaboração, revisão e melhoria contínua dos demais, e
-- formaliza a resposta a mudanças de legislação, fiscalizações e orientações
-- da Vigilância Sanitária.
--
-- NOTA: esta versão não usa ponto-e-vírgula dentro do texto nem bloco DO $$,
-- para que o editor SQL do Supabase não exiba o falso aviso de
-- "UPDATE sem cláusula WHERE". Os dois comandos são seguros:
--   1) o INSERT só grava se o POP-FAR-001 ainda não existir
--   2) o UPDATE atinge somente a linha com codigo = 'POP-FAR-001'
--
-- Depende de: migration_pops.sql e migration_pops_corpo.sql aplicadas.
-- Pode ser executado mais de uma vez sem duplicar.
-- ============================================================================

-- 1) Cria o POP-FAR-001 apenas se ainda não existir
insert into pops (area, titulo, status, ordem, is_dado_teste, codigo, versao)
select 'Farmácia', 'Sistema de Gestão da Qualidade da Farmácia', 'pendente', 0, false, 'POP-FAR-001', '01'
where not exists (select 1 from pops where codigo = 'POP-FAR-001');

-- 2) Grava o conteúdo do POP-FAR-001
update pops set
  area = 'Farmácia',
  ...
  corpo = $J${
    ...
    "aplicacao": "Aplica-se a todos os POPs e registros da farmácia e às atividades correlatas executadas pela enfermagem sob orientação farmacêutica. É o procedimento mestre do conjunto documental, ao qual os demais POPs se subordinam quanto às regras de elaboração, revisão e controle. A condução do SGQ é atribuição do farmacêutico Responsável Técnico, em regime presencial periódico com retaguarda à distância à equipe.",
    "responsabilidades": [
    ...
    "procedimento": [
      "ELABORAÇÃO — Cada procedimento é redigido descrevendo o processo tal como efetivamente executado no serviço, com identificação (código, versão, data de vigência, próxima revisão e responsável) e as seções padronizadas: objetivo, campo de aplicação, responsabilidades, materiais, procedimento, registros e referências.",
      ...
      "REVISÃO EXTRAORDINÁRIA — Independentemente do prazo programado, o POP é revisado sempre que ocorrer uma destas situações: alteração da legislação sanitária aplicável, orientação ou exigência de autoridade sanitária, mudança na estrutura, no quadro de pessoal ou no fluxo assistencial do serviço, identificação de não conformidade, incidente ou quase-falha relacionada ao processo, ou sugestão da equipe que aprimore a segurança e a rastreabilidade.",
      ...
      "AUTOAVALIAÇÃO PERIÓDICA — O RT realiza, quando presencial, verificação do cumprimento dos procedimentos e da suficiência dos registros, tais como escrituração, mapas de medicação, termos, conferências de estoque e do carrinho de emergência, corrigindo os desvios identificados.",
      "MELHORIA CONTÍNUA — Os resultados das revisões, das não conformidades tratadas e das autoavaliações realimentam o conjunto documental, de modo que o SGQ evolua junto com o serviço, mantendo a documentação sempre aderente à realidade assistencial e à norma vigente."
    ],
    "registros": [
      "Registro Mestre de POPs, com relação, versões, vigências e próximas revisões",
